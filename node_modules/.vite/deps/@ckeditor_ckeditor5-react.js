...
function isCKZeroBaseVersion(version) {
  return !!(version == null ? void 0 : version.startsWith("0.0.0-"));
}
...
function compareInstalledCKBaseVersion(version) {
  var _a;
  const installedVersion = (_a = getCKBaseBundleInstallationInfo()) == null ? void 0 : _a.version;
  if (!installedVersion) {
    return null;
  }
...
function assignAttributesPropToMultiRootEditorConfig(attributes, config) {
  ...
  if (supports.rootsConfigEntry) {
    ...
    const roots = knownRootsKeys.reduce((acc, rootName) => {
      var _a, _b;
      const legacyRootAttributes = (_a = config.rootsAttributes) == null ? void 0 : _a[rootName];
      const configRootValue = (_b = config.roots) == null ? void 0 : _b[rootName];
      acc[rootName] = {
        ...configRootValue,
        modelAttributes: (attributes == null ? void 0 : attributes[rootName]) ?? {
          ...legacyRootAttributes,
          ...configRootValue == null ? void 0 : configRootValue.modelAttributes
        }
      };
      return acc;
...
function assignInitialDataToMultirootEditorConfig(data, config) {
  ...
  if (supports.rootsConfigEntry) {
    ...
    const roots = knownRootsKeys.reduce((acc, rootName) => {
      var _a, _b;
      const rootConfig = (_a = config.roots) == null ? void 0 : _a[rootName];
      const rootInitialData = rootConfig == null ? void 0 : rootConfig.initialData;
      if (rootInitialData && (data == null ? void 0 : data[rootName])) {
        console.warn(
      ...
      acc[rootName] = {
        ...rootConfig,
        initialData: rootInitialData || (data == null ? void 0 : data[rootName]) || ((_b = config.initialData) == null ? void 0 : _b[rootName]) || ""
      };
      return acc;
    }, /* @__PURE__ */ Object.create(null));
  ...
  if (data && (config == null ? void 0 : config.initialData)) {
    console.warn(
  ...
  return {
    ...config,
    initialData: (config == null ? void 0 : config.initialData) || data
  };
}
function assignElementToEditorConfig(Editor, element, config) {
  ...
  const mappedConfig = {
    ...
    roots: {
      ...
      main: {
        ...config.root,
        ...(_a = config.roots) == null ? void 0 : _a.main,
        element
      }
    }
  };
...
function getInitialDataFromEditorConfig(config) {
  ...
  const supports = getInstalledCKBaseFeatures();
  if (supports.rootsConfigEntry) {
    return ((_b = (_a = config.roots) == null ? void 0 : _a.main) == null ? void 0 : _b.initialData) || ((_c = config.root) == null ? void 0 : _c.initialData) || /* legacy */
    config.initialData;
  }
...
var useCKEditorWatchdogContext = () => (0, import_react.useContext)(ContextWatchdogContext);
...
function normalizeEditorElementDefinition(definition) {
  if (typeof HTMLElement !== "undefined" && definition instanceof HTMLElement) {
    throw new Error(
      "An HTMLElement cannot be used as an editor element definition. Please pass a string, a React component, or an object definition."
    );
  }
...
var EditorElement = (0, import_react.memo)((0, import_react.forwardRef)(({ definition }, ref) => {
  const { name: Tag, classes, styles, attributes } = normalizeEditorElementDefinition(definition != null ? definition : {
    name: "div"
  });
...
var useLifeCycleSemaphoreSyncRef = () => {
  ...
  const pendingWaiters = (0, import_react.useRef)([]);
  const refresh = () => {
    setRevision(Date.now());
  };
  const release = (rerender = true) => {
    var _a;
    (_a = semaphoreRef.current) == null ? void 0 : _a.release();
  ...
  const runAfterMount = (callback) => {
    var _a;
    (_a = semaphoreRef.current) == null ? void 0 : _a.runAfterMount(callback);
  };
  ...
  const waitFor2 = () => {
    var _a;
    const currentValue = (_a = semaphoreRef.current) == null ? void 0 : _a.value;
    if (currentValue) {
...
var EditorEditable = (0, import_react.memo)((0, import_react.forwardRef)(({ id, editor, rootName }, ref) => {
  ...
  const rootEditableOptions = (0, import_react.useMemo)(() => {
    ...
    const options = root.getAttribute(ROOT_EDITABLE_OPTIONS_ATTRIBUTE);
    return { ...options };
  }, [root]);
  (0, import_react.useEffect)(() => {
  ...
  let normalizedDefinition = normalizeEditorElementDefinition((_a = rootEditableOptions == null ? void 0 : rootEditableOptions.element) != null ? _a : {
    name: "div"
  });
...
var useMultiRootEditor = (props) => {
  ...
  const removeRoot = async (rootName) => {
